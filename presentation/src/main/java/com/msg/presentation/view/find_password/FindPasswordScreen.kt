...
fun FindPasswordScreen(
    ...
    FindPasswordViewModelProvider(viewModelStoreOwner = viewModelStoreOwner) { findPasswordViewModel ->
        ...
        val changePasswordState by findPasswordViewModel.changePasswordState.collectAsState()

        when (changePasswordState) {
            is Event.Success -> {
                navigateToLogin()
                findPasswordViewModel.initChangePassword()
            }
            else -> {}
        }
        ...
        Column(
            ...
            DotoriButton(
                ...
                if (passwordText != checkPasswordText) {
                    ...
                    findPasswordViewModel.password.value = passwordText
                    findPasswordViewModel.changePassword(
                        ChangePasswordRequestModel(
                            email = findPasswordViewModel.email.value,
                            newPassword = findPasswordViewModel.password.value
                        )
                    )
                }
            }
